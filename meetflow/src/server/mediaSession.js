import presenceRef from "./realtimeBackend";
import { store } from "../index";

const participantRef = presenceRef.child("participants");
...
export const initializeListensers = async (userId) => {
  ...
  currentUserRef.child("offers").on("child_added", async (snapshot) => {
    ...
    if (data?.offer) {
      const pc =
        store.getState().participantDirectory[data.offer.userId].peerConnection;
  ...
  currentUserRef.child("offerCandidates").on("child_added", (snapshot) => {
    ...
    if (data.userId) {
      const pc =
        store.getState().participantDirectory[data.userId].peerConnection;
  ...
  currentUserRef.child("answers").on("child_added", (snapshot) => {
    ...
    if (data?.answer) {
      const pc =
        store.getState().participantDirectory[data.answer.userId]
          .peerConnection;
...
const createAnswer = async (otherUserId, userId) => {
  const pc =
    store.getState().participantDirectory[otherUserId].peerConnection;
